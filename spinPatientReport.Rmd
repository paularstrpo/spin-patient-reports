...
params:
  ...
  dateSampleCollected: '01/02/2021'
  dateSampleSequenced: '04/20/2021'
  dateReportGenerated:  !r Sys.Date()
  ...
  msiFile: !r NA
  seliFile: 'exampleInputs/selinescores_latest.tsv'
  ...
  mmPSNFile: 'exampleInputs/ISMMS01/Predicted_class.csv'
  translocFile: !r NA
  drugRepurposingFile: !r NA
  pathwayFile: 'exampleInputs/MM_5130_T_03/pathways.prediction_engine.results.tsv'
  ...
  tumorPurityPloidy: 'exampleInputs/ISMMS01/tumor.facets_output.txt'
  facetsCNCFResultsFile: 'exampleInputs/ISMMS01/tumor.facets_cncf.txt'
  mmPSNPlotListData: 'refData/mmPSN_survivalCurvePlotList.rds'
  geneCoordinateBED: 'refData/hg38_geneCoordinates_TxDB.tsv'
---

<img src="/report-source/MS_RGB_Hrztl.png" width="250" height="193" style="position:absolute;top:0px;right:0px;" />
...
colors <- c(`Somatic Mutation`="black",
            ...
            `Downregulated Gene Expression`="#15D6D6", 
            `Upregulated Gene Expression`="#00AEEF", 
...
expr.dist <- function(gene){ # x = gene symbol
  ...
  df <- data.frame(sampleName=dat$sampleName, zscore=dat[, gene], geneName=gene,
...
if(!is.na(exprRes)){
  exprRes$Variant <- ifelse(as.numeric(exprRes$Zscore) > 1.5, 'Upregulation', 'Downregulation')
  
  exprsTable <- left_join(unique(exprRes[, c('Hugo_Symbol', 'Variant', 'Zscore')]), geneCoords, by=c('Hugo_Symbol'='Gene'))
  
  exprBED <- data.frame(chr=exprsTable$seqnames,
                        start=exprsTable$start,
                        end=exprsTable$start,
                        gene=paste(exprsTable$Hugo_Symbol, gsub('regulation', '', exprsTable$Variant) ),
                        type=paste(gsub('ion', 'ed', exprsTable$Variant), 'Gene Expression'))
...
pa## Drug Recommendation Summary

:::{style="display: grid; grid-template-columns: 1fr 1fr; grid-column-gap: 10px;"}

```{r rx-bucket-summary-kable, results='asis'}
bucketSummaryTable <- drugDetailsRankedTable %>% 
  inner_join(rxBucketSummaryTable) %>%
  inner_join(variantSupportSummaryTable) %>%
  filter(bucket.summary_score > 0 & rx.summary_score > 0) %>%
  arrange(-bucket.summary_score) %>% 
  dplyr::select(Rx.Bucket, Rx.Reccomendation, variant_name, variant_statement) %>%
  unique() %>% 
...
for(bucket in unique(bucketSummaryTable$Rx.Bucket)){
  cat(paste0('### ', bucket, '\n\n'))
  
  statement <- bucketSummaryTable %>% filter(Rx.Bucket == bucket)
...
for(bucket in bucketSummaryTable$Rx.Bucket){
  cat(paste0('### **', bucket, '**\n'))
  
  ranked_drugs <- rxBucketSummaryTable %>% filter(Rx.Bucket == bucket) %>% arrange(-rx.summary_score)
  ...
  for(rx in ranked_drugs$Rx.Reccomendation){
    ...
    evidence <- variantSupportSummaryTable %>% 
      filter(Rx.Reccomendation == rx) %>% group_by(Hugo_Symbol) %>%
      summarise(statement =  paste(
                    ...
                    '\n', collapse='\n')) %>%
      mutate(statement = paste(paste0('##### ***', Hugo_Symbol, '***\n'), statement, collapse='\n\n'))
